...
| 8. Engagement | Favorite, ActivityHistory, Notification, StudyReminder, Search | 🔄 Đang thực hiện — Favorite + ActivityHistory + Notification/StudyReminder xong, xem mục 13 |
...
Còn lại theo roadmap: `Search`; sau đó Giai đoạn 9 (Admin & Analytics), Giai đoạn 10 (Production), cộng với `Tag`/`VocabularyTag`/`VocabularyRelation` + `Achievement`/`Leaderboard` (Phase 2, hoãn từ Giai đoạn 3/7).

Bước tiếp theo: `Search` (`GET /api/search?q=&type=`, tìm kiếm Course/Lesson/Vocabulary/Grammar/Public Deck — đóng nốt Giai đoạn 8), rồi Giai đoạn 9 (Admin & Analytics).
