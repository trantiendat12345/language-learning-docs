...
Bước tiếp theo: **Review** (trang ôn tập SRS ngắt quãng, dùng `GET /api/review/today` liệt kê từ đến hạn ôn hôm nay + `POST /api/review/{vocabularyId}` chấm Forgot/Hard/Good/Easy — tái dùng `reviewService` đã có, khác Vocabulary Learning/Flashcard ở chỗ nguồn từ không cố định theo 1 Lesson/Deck mà gộp mọi từ user đã học tới hạn ôn).
