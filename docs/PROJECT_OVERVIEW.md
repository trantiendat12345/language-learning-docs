...
| POST | `/api/quizzes/generate` | protected — ✅ đã implement, body: `{sourceType, sourceId, questionCount}` (`questionCount` bỏ trống = Tất cả). Chunk hiện tại chỉ hỗ trợ `sourceType=LESSON` (400 nếu COURSE/DECK/VOCAB_LIST — Deck chưa tồn tại tới Giai đoạn 5) |
...
Còn lại theo roadmap: Giai đoạn 5 (Deck), Giai đoạn 6 (Spaced Repetition), Giai đoạn 7 (Progress & Gamification — bao gồm XP/Streak/`XpLog`/D8 đang hoãn từ Giai đoạn 3+4), Giai đoạn 8+ (Engagement/Admin Analytics/Production), cộng với `Tag`/`VocabularyTag`/`VocabularyRelation` (Phase 2, hoãn từ Giai đoạn 3).

Bước tiếp theo: Giai đoạn 5 — `Deck` + `DeckCard` (CRUD, Public/Private, Clone, add/remove vocabulary) — phụ thuộc `Vocabulary` đã có, đồng thời sẽ bổ sung `sourceType=DECK` cho Quiz generate (hiện đang trả 400 vì Deck chưa tồn tại).
