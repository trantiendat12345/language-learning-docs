...
| 5. Deck | Deck, DeckCard, Public/Private, Clone, Flashcard learning modes | 🔄 Đang thực hiện — Deck CRUD + DeckCard (add existing/custom word, remove) + Public search + Clone xong. Flashcard learning modes (Normal/Reverse/Shuffle) + đánh giá Forgot/Hard/Good/Easy hoãn sang Giai đoạn 6 vì cần `UserVocabularyProgress` |
...
Còn lại theo roadmap: nốt Flashcard/SRC của Giai đoạn 5, Giai đoạn 6 (Spaced Repetition đầy đủ), Giai đoạn 7 (Progress & Gamification — bao gồm XP/Streak/`XpLog`/D8 đang hoãn từ Giai đoạn 3+4), Giai đoạn 8+ (Engagement/Admin Analytics/Production), cộng với `Tag`/`VocabularyTag`/`VocabularyRelation` (Phase 2, hoãn từ Giai đoạn 3).

Bước tiếp theo: Giai đoạn 6 — Spaced Repetition (`UserVocabularyProgress`, `ReviewLog`, thuật toán SM-2, `GET /api/review/today`, `POST /api/review/{vocabularyId}`) — đồng thời sẽ hoàn thiện nốt Flashcard learning modes của Giai đoạn 5 (đánh giá Forgot/Hard/Good/Easy ghi vào `UserVocabularyProgress` vừa xây).
