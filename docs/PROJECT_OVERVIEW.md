...
├── layouts/         (PublicLayout ✅, AuthLayout ✅, UserLayout ✅, AdminLayout ✅ mới — sidebar điều hướng, không dùng chung Navbar)
├── mock/            (dashboardMock.ts ✅ — mock data riêng cho phần Dashboard chưa có API thật, xem mục 8.1)
├── pages/           (pages/auth/{LoginPage,RegisterPage,ForgotPasswordPage,ResetPasswordPage,VerifyEmailPage}.tsx ✅ — đã redesign theo Design System 2026-08-06, pages/auth/AuthForm.module.scss dùng chung, DashboardPage.tsx ✅ — đã redesign 2026-08-06 (dữ liệu thật qua ProgressDashboard + History + Course API, chỉ Achievement/Leaderboard dùng mock), pages/courses/{CourseListPage,CourseDetailPage}.tsx ✅ — đã redesign 2026-08-06 (hero banner + lesson list + sticky enroll card, dữ liệu thật), pages/lessons/LessonDetailPage.tsx ✅ — mới 2026-08-07 (Vocabulary/Grammar/Complete, gating preview theo `enrolled`), pages/lessons/VocabularyLearningPage.tsx ✅ — mới 2026-08-07 (route `/lessons/:id/vocabulary`, học từ vựng theo Lesson, đánh dấu đã học qua `POST /api/review/{vocabularyId}`), pages/decks/{DeckListPage,DeckDetailPage,FlashcardPage}.tsx ✅ — mới 2026-08-07 (Khám phá/Deck của tôi/Tạo Deck, chi tiết Deck + CRUD thẻ + Clone, Flashcard lật thẻ 3D Xuôi/Ngược/Xáo trộn), pages/lessons/QuizPage.tsx ✅ — mới 2026-08-08 (route `/lessons/:id/quiz`, setup số câu + làm bài + kết quả, generate động qua Lesson), pages/QuizHistoryPage.tsx ✅ — mới 2026-08-08 (route `/quiz-history`, lịch sử làm Quiz dạng accordion), pages/ReviewPage.tsx ✅ — mới 2026-08-08 (route `/review`, ôn tập SRS ngắt quãng gộp mọi từ đến hạn, không cố định theo Lesson/Deck), ProfilePage.tsx ✅ — đã redesign 2026-08-08 (header avatar/stats, form sửa hồ sơ + đổi mật khẩu theo Design System), pages/admin/{AdminDashboardPage,AdminUserListPage}.tsx ✅ — mới 2026-08-08 (route `/admin`, `/admin/users`, trong `AdminRoute`), pages/admin/{AdminLanguageListPage,AdminCourseListPage,AdminCourseDetailPage,AdminLessonDetailPage}.tsx ✅ — mới 2026-08-08 (route `/admin/languages`, `/admin/courses`, `/admin/courses/:id`, `/admin/lessons/:id` — CRUD Ngôn ngữ/Khoá học/Bài học qua giao diện, thay cho seed bằng curl), LandingPage.tsx ✅ — đã redesign 2026-08-08 (hero gradient+blob, feature grid, Khoá học nổi bật dữ liệu thật qua `GET /api/courses`, CTA đổi theo trạng thái đăng nhập) — **chunk cuối của Redesign Frontend theo Design System**)
├── routes/           (AppRoutes.tsx ✅, PublicRoute.tsx ✅, ProtectedRoute.tsx ✅, AdminRoute ✅ mới — check `isAdmin` từ `AuthContext`, redirect `/login` nếu chưa đăng nhập hoặc `/dashboard` nếu không phải ADMIN)
├── services/         (authService.ts ✅, userService.ts ✅ đủ 3 method /api/users/me, courseService.ts ✅, languageService.ts ✅, progressService.ts ✅ (GET /api/progress/dashboard), historyService.ts ✅ (GET /api/history/recent), lessonService.ts ✅, vocabularyService.ts ✅ (GET /api/vocabularies/{id}), reviewService.ts ✅ (submitReview + getTodayReview mới bổ sung GET /api/review/today), deckService.ts ✅ (đủ 10 method /api/decks/**), quizService.ts ✅ (generateQuiz/submitQuiz/getAttempts/getAttemptById), adminService.ts ✅ mới (getDashboard/getUsers/getUserById/getUserProgress/activateUser/disableUser/lockUser)...)
├── types/            (api.ts ✅, auth.ts ✅, user.ts ✅ — khớp 1-1 Response/Request DTO backend, progress.ts ✅ bổ sung ProgressDashboardResponse/ContinueLearningResponse, history.ts ✅ mới, lesson.ts ✅, grammar.ts ✅, vocabulary.ts ✅, review.ts ✅ bổ sung ReviewTodayItemResponse, deck.ts ✅, quiz.ts ✅, admin.ts ✅ mới)
├── constants/
...
- **Quyết định phạm vi**: redesign lại cả trang đã có sẵn (không chỉ trang mới) để tránh giao diện nửa cũ nửa mới — **đã hoàn thành toàn bộ 12 màn hình theo đúng thứ tự đã chốt**: ~~Dashboard~~ → ~~Course List~~ → ~~Course Detail~~ → ~~Lesson~~ → ~~Vocabulary Learning~~ → ~~Flashcard~~ → ~~Quiz~~ → ~~Review~~ → ~~Profile~~ → ~~Admin Dashboard~~ → ~~Landing Page~~ (2026-08-08). Ngoài 12 màn hình gốc, phát sinh thêm redesign `Navbar` (bug layout phát hiện sau khi có đủ nhiều mục header).
...
- **Chunk 8 — Quiz** (`pages/lessons/QuizPage.tsx` route `/lessons/:id/quiz`, `pages/QuizHistoryPage.tsx` route `/quiz-history`, đều public). Không cần hỏi lại phạm vi (khác Deck/Flashcard) vì Backend đã hỗ trợ trọn vẹn generate→submit→lịch sử cho `sourceType=LESSON`, điểm vào tự nhiên là nút "Làm Quiz" mới thêm ở header `LessonDetailPage`.
  - `types/quiz.ts`, `services/quizService.ts` (mới, đủ 4 method khớp `QuizController`: `generateQuiz`/`submitQuiz`/`getAttempts`/`getAttemptById`).
  - **`QuizPage`**: state machine 3 giai đoạn `setup → taking → result` trong 1 component. Setup chọn số câu 10/20/50/Tất cả (diễn giải đúng `questionCount: number | undefined`). Sau generate, nếu `requestedCount > actualCount` hiện banner cảnh báo "chỉ có X câu, đã dùng toàn bộ X câu" (2 field Backend trả về đúng mục đích thiết kế cho FE tự so sánh). Mỗi câu render theo `type`: `MULTIPLE_CHOICE/IMAGE_CHOICE/AUDIO_CHOICE` → danh sách option chọn 1; `FILL_BLANK/TYPING` → ô nhập text tự do; `LISTENING/MATCHING/REORDER` → chưa hỗ trợ làm bài (đúng giới hạn Backend nêu ở mục 12 Giai đoạn 4), chỉ hiện ghi chú + cho bỏ qua. Nút "Bỏ qua câu này" luôn bấm được (gửi answer rỗng cho câu đó, đúng hành vi Backend "để trống = tính sai, không lỗi"). Nộp bài xong hiện ngay kết quả (Backend trả `answers[]` kèm `correctOptionId`/`explanation`/`correct` từng câu ngay trong response submit).
  - **Giới hạn Backend phát hiện khi code, cần lưu ý cho UI xem lại bài**: `QuizAttemptAnswerResponse` chỉ trả `correctOptionId` dạng **id**, không có text — với câu trắc nghiệm FE tự map sang text nhờ vẫn giữ `questions[]` (kèm `options[]`) từ lúc generate trong state của phiên hiện tại; nhưng với `FILL_BLANK/TYPING` thì Backend không trả text đáp án đúng ở bất kỳ đâu (`options` rỗng lúc generate), nên màn kết quả **chỉ hiện đúng/sai** cho 2 loại này, không hiện được "đáp án đúng là...". Giới hạn này nặng hơn ở `QuizHistoryPage` (xem bên dưới) vì không còn `questions[]` gốc để tra cứu.
  - **`QuizHistoryPage`**: danh sách lịch sử (`GET /api/quizzes/attempts`, phân trang, sort `completedAt` giảm dần) dạng accordion — bấm 1 dòng để lazy-load chi tiết (`GET /api/quizzes/attempts/{id}`, cache theo `attemptId` tránh gọi lại). Tên bài học hiển thị bằng cách enrich song song `GET /api/lessons/{id}` cho từng `sourceId` duy nhất trong trang hiện tại (cùng pattern `Promise.allSettled` đã dùng ở chunk Vocabulary Learning), fallback "Bài học #id" nếu fetch lỗi. **Vì đây là phiên làm bài trong quá khứ, không còn `questions[]` gốc** → phần chi tiết accordion chỉ hiện được prompt + đúng/sai + explanation, **không** hiện được nội dung lựa chọn đã chọn/đáp án đúng cho câu trắc nghiệm (khác `QuizPage` ngay sau khi nộp bài, lúc đó vẫn còn `questions[]` trong state) — có ghi chú rõ giới hạn này ngay trên UI để không gây hiểu nhầm là bug.
  - Thêm nút "Làm Quiz" ở header `LessonDetailPage` (luôn hiện cho mọi Lesson, **không** gate theo `enrolled` vì Backend `POST /api/quizzes/generate` không yêu cầu Enrollment — chỉ cần Lesson+Course đã `PUBLISHED` và user đã đăng nhập).
  - Test E2E qua Playwright + Backend thật (seed Language+Course+Lesson+4 Question: 2 `MULTIPLE_CHOICE`, 1 `FILL_BLANK`, 1 `TYPING`): xác nhận prompt đăng nhập khi chưa login, nút "Làm Quiz" ở LessonDetailPage, làm đủ 4 câu (3 đúng cố ý + 1 sai cố ý ở câu TYPING) ra đúng kết quả 75% (3/4), từng câu hiện đúng/sai + giải thích, lịch sử hiện đúng tên bài học + accordion chi tiết đúng, banner cảnh báo khi chọn 20 câu nhưng nguồn chỉ có 4 câu. **Lưu ý khi viết test**: Backend `Collections.shuffle` cả thứ tự câu hỏi lẫn thứ tự option mỗi lần generate — script test ban đầu giả định thứ tự cố định gây báo sai (không phải bug sản phẩm), sửa bằng cách đọc prompt hiện tại rồi tự quyết định hành động thay vì hard-code theo vị trí. Light/dark/mobile đều đúng. Đã dọn sạch dữ liệu test (2 user + Language/Course/Lesson/4 Question/2 QuizAttempt), xác nhận không đụng 4 tài khoản có sẵn từ trước.
  - Commit: client `[V18]`, docs `[V37]` (dự kiến — chờ xác nhận từ người dùng trước khi commit/push).

- **Chunk 9 — Review** (`pages/ReviewPage.tsx`, route `/review`, public). Đúng như dự kiến ở cuối chunk trước: khác Vocabulary Learning/Flashcard ở chỗ nguồn từ **không cố định theo 1 Lesson/Deck** mà gộp mọi từ user đã học tới hạn ôn trong toàn hệ thống (`GET /api/review/today`).
  - `types/review.ts` bổ sung `ReviewTodayItemResponse` (khớp response GET /today: `vocabularyId/word/meaning/ipa/imageUrl/wordType/nextReviewDate/masteryLevel`), `services/reviewService.ts` bổ sung `getTodayReview()` — tái dùng nguyên `submitReview()` đã có từ chunk Vocabulary Learning/Flashcard, không viết lại logic gọi `POST /api/review/{vocabularyId}`.
  - UI: thẻ lật 3D tái dùng đúng pattern CSS đã có ở `FlashcardPage` (không tách thành component dùng chung — nhất quán với cách các chunk trước cũng tự viết SCSS riêng thay vì trừu tượng hoá sớm khi chỉ có 2 nơi dùng). **Khác Flashcard**: chỉ 1 chiều duy nhất (front luôn là từ, back luôn là nghĩa — không có mode Xuôi/Ngược/Xáo trộn) và **giữ nguyên thứ tự Backend trả về** (quá hạn lâu nhất trước) thay vì cho phép xáo trộn, vì thứ tự ưu tiên ôn từ quá hạn lâu là chủ đích thiết kế SRS, xáo trộn sẽ phá vỡ mục đích đó. 4 nút đánh giá Forgot/Hard/Good/Easy giống hệt Flashcard. Rỗng (không có từ đến hạn) hiện empty state khích lệ "Bạn đã ôn hết rồi!" kèm CTA sang Deck học thêm từ mới, không phải lỗi.
  - **Cải thiện nhỏ đi kèm (đóng vòng UX đã bỏ ngỏ từ chunk Dashboard)**: `StatTile` (`components/dashboard/StatTile.tsx`) bổ sung prop `to?` tuỳ chọn — tile "Từ cần ôn tập" trên Dashboard giờ bấm được, điều hướng thẳng sang `/review` (trước đây hiển thị số nhưng không làm gì được). Thêm link "Ôn tập" vào `Navbar` (chỉ hiện khi đã đăng nhập, giống cách `Dashboard`/`Profile` cũng chỉ hiện khi có `user`).
  - Áp dụng đúng quy tắc `isAuthLoading` gating (CODING_CONVENTIONS.md mục 2.2) vì `GET /api/review/today` yêu cầu đăng nhập hoàn toàn.
  - Test E2E qua Playwright + Backend thật (seed Language + 3 Vocabulary, gọi `POST /api/review/{id}` 1 lần cho mỗi từ để tạo `UserVocabularyProgress` rồi **chỉnh trực tiếp `next_review_date` qua SQL** thành quá khứ/hôm nay/tương lai cho 3 từ — vì API luôn đặt lịch ôn tiếp theo ra tương lai, không có cách nào qua API tạo sẵn trạng thái "đến hạn hôm nay" để test): xác nhận đúng 2/3 từ hiển thị (từ tương lai bị loại đúng), đúng thứ tự quá hạn lâu nhất trước, bấm stat tile Dashboard điều hướng đúng sang `/review`, lật thẻ + đánh giá cả 2 từ ra đúng màn hoàn thành kèm thống kê theo mức đánh giá, vào lại trang sau khi ôn hết hiện đúng empty state (không phải lỗi). Light/dark/mobile đều đúng. Đã dọn sạch dữ liệu test (2 user + Language/3 Vocabulary/review_log/user_vocabulary_progress liên quan), xác nhận không đụng 4 tài khoản có sẵn từ trước.
  - Commit: client `[V19]`, docs `[V38]` (dự kiến — chờ xác nhận từ người dùng trước khi commit/push).

- **Chunk 10 — Profile** (redesign `pages/ProfilePage.tsx`, route `/profile` trong `ProtectedRoute`). Trang cuối cùng của luồng học chính trước khi chuyển sang Admin Dashboard rồi Landing Page.
  - **Phát hiện + sửa 1 bug thật trước khi redesign UI, không phải lỗi nhỏ**: `types/user.ts` (`UserResponse`/`UserUpdateRequest`) thiếu hẳn `dailyGoalType`/`dailyGoalValue` dù 2 field này đã được Backend thêm vào `User` entity + bắt buộc trong `UserUpdateRequest` (`@NotNull`/`int` primitive) từ chunk Giai đoạn 7 (Progress & Gamification) — `ProfilePage.tsx` gốc từ Giai đoạn 2 chưa từng được cập nhật theo, nghĩa là nút "Lưu thay đổi" **luôn bị Backend trả 400 VALIDATION_ERROR** kể từ khi Giai đoạn 7 hoàn thành cho tới tận chunk này (không ai phát hiện vì FE chưa được test tay lại từ đó). Sửa bằng cách bổ sung đủ 2 field vào `types/user.ts` (tái dùng `DailyGoalType` đã có sẵn ở `types/progress.ts`, không định nghĩa trùng), thêm 2 field tương ứng vào form (Select loại mục tiêu + Input số lượng, nhãn đơn vị đổi động theo loại đã chọn nhờ `useWatch`). Verify qua Playwright: bắt response `PUT /api/users/me` trả đúng `200` (trước đây sẽ là `400`), dữ liệu còn nguyên sau khi reload trang (xác nhận lưu DB thật, không phải optimistic UI đánh lừa).
  - UI: header card (avatar thật hoặc fallback chữ cái đầu trên nền gradient, tên hiển thị, email, badge XP/Streak/Trình độ), 2 card form riêng (Sửa hồ sơ, Đổi mật khẩu) dùng `Input`/`Select`/`Button`/`Card` có sẵn — không có component `ui/` mới. Giữ nguyên toàn bộ logic validate (`react-hook-form`, password pattern, confirm khớp) từ bản gốc, chỉ đổi lớp trình bày.
  - **Cải thiện nhỏ đi kèm**: thêm `to` prop cho `StatTile` (đã làm ở chunk Review) chưa cần dùng thêm ở đây, không thay đổi gì thêm ngoài phạm vi Profile.
  - Test E2E qua Playwright + Backend thật: sửa đủ các field (tên hiển thị, trình độ, đổi loại+giá trị mục tiêu hằng ngày) lưu thành công + hiển thị đúng ngay + còn đúng sau reload, đổi mật khẩu test cả nhánh lỗi (confirm không khớp hiện đúng thông báo) lẫn nhánh thành công. Light/dark/mobile đều đúng (form 2 cột rút về 1 cột đúng breakpoint). Đã dọn sạch user test, xác nhận không đụng 4 tài khoản có sẵn từ trước.
  - Commit: client `[V20]`, docs `[V39]` (dự kiến — chờ xác nhận từ người dùng trước khi commit/push).

- **Chunk 11 — Admin Dashboard** (`pages/admin/{AdminDashboardPage,AdminUserListPage}.tsx`, route `/admin`, `/admin/users`, bọc trong `AdminRoute`+`AdminLayout` mới). **Chốt phạm vi trước khi code** (hỏi người dùng qua AskUserQuestion vì `/api/admin/**` có rất nhiều nhóm API — Dashboard, User management, và CRUD nội dung Language/Course/Lesson/Vocabulary/Grammar/Question — làm hết trong 1 chunk sẽ lớn hơn bất kỳ chunk nào trước đó): chọn **Dashboard + Quản lý User**, đúng nghĩa đen tên màn hình trong danh sách 12 gốc; CRUD nội dung để dành chunk riêng sau nếu cần, vẫn seed qua curl như mọi chunk trước.
  - **Phát hiện 1 vấn đề kiến trúc quan trọng khi khảo sát API trước khi code**: `UserResponse` (dùng cho `GET /api/users/me`) **không có field `role`/`roles`** — Backend hoàn toàn không có endpoint/field JSON nào trả về role của user hiện tại. Cách duy nhất FE biết user có phải ADMIN hay không là **tự decode claim `roles: string[]`** trong payload JWT access token (`JwtService.generateAccessToken` có set claim này, giá trị `["ADMIN"]`/`["USER"]`/cả hai). Xử lý bằng cách thêm `getRolesFromToken()` vào `api/tokenStore.ts` (tự viết base64url-decode + `JSON.parse`, **không cài thêm package** như `jwt-decode` vì chỉ cần đọc claim, không cần verify signature — verify thật luôn nằm ở Backend `SecurityConfig.hasRole("ADMIN")`), `AuthContext` thêm state `isAdmin` cập nhật đồng bộ mỗi khi `accessToken` đổi (login/restoreSession/logout).
  - `AdminRoute` (mới, `routes/AdminRoute.tsx`) — chặn theo 2 tầng: chưa đăng nhập → `/login`, đã đăng nhập nhưng không phải ADMIN → `/dashboard` (không lộ trang admin tồn tại cho user thường, nhất quán tinh thần "không tiết lộ" đã áp dụng xuyên suốt Backend). `AdminLayout` (mới) — sidebar điều hướng riêng (Tổng quan/Người dùng + Về trang chính/Đăng xuất/ThemeToggle), **cố tình không dùng chung `Navbar`** vì `Navbar` hiện vẫn là Bootstrap mặc định từ Giai đoạn 2 (chưa nằm trong scope redesign nào), còn Admin cần chrome quản trị riêng biệt rõ ràng với phần người dùng thường.
  - `types/admin.ts`, `services/adminService.ts` (mới, đủ 7 method khớp `AdminDashboardController`+`AdminUserController`). Thêm variant `danger` cho `Badge` (dùng `--dl-danger-*` token có sẵn) — trước đó Badge chỉ có 5 variant, không đủ để phân biệt trạng thái User `DISABLED`/`LOCKED` màu đỏ với `ACTIVE` màu xanh.
  - **`AdminDashboardPage`**: 7 thẻ thống kê tổng quan (`GET /api/admin/dashboard`), tái dùng thẳng `StatTile` đã có ở `components/dashboard/` (đủ tổng quát, tránh tạo trùng component) — 2 thẻ liên quan User (`totalUsers`/`activeUsers`) bấm được, điều hướng sang `/admin/users` nhờ prop `to` đã thêm ở chunk Review.
  - **`AdminUserListPage`**: tìm kiếm theo username/email (`GET /api/admin/users?keyword=`, phân trang), mỗi dòng dạng accordion — bấm mở rộng lazy-load tiến độ (`GET /api/admin/users/{id}/progress`, cache theo id) hiển thị XP/Streak/danh sách `CourseEnrollment`. Nút Vô hiệu hoá/Khoá/Kích hoạt lại hiện đúng theo `status` hiện tại (`ACTIVE` → Vô hiệu hoá+Khoá; `DISABLED`/`LOCKED` → Kích hoạt lại), có `window.confirm` cho 2 hành động mang tính phá vỡ. **Chủ động chặn từ UI, không đợi Backend trả lỗi**: nếu dòng đang xem là chính currentAdmin (`user.id === currentUser.id`), ẩn hết nút hành động và hiện ghi chú giải thích — khớp đúng rule `AdminUserServiceImpl.requireNotSelf()` (400 `BAD_REQUEST` nếu cố tự đổi trạng thái chính mình), tránh để admin bấm phải nút chắc chắn lỗi.
  - Thêm link "Admin" vào `Navbar` (chỉ hiện khi `isAdmin === true`).
  - Test E2E qua Playwright + Backend thật (1 admin + 2 user thường, 1 user đã ghi danh 1 Course để có dữ liệu tiến độ hiển thị): xác nhận user thường vào `/admin` bị đá về `/dashboard` và **không thấy link Admin trên Navbar**, chưa đăng nhập vào `/admin` bị đá về `/login`, admin thấy link Admin + vào được Dashboard + số liệu đúng, tìm kiếm User đúng, mở rộng xem tiến độ hiện đúng Enrollment vừa tạo, Vô hiệu hoá → badge đổi đúng màu đỏ → Kích hoạt lại → badge về xanh đúng, tìm chính currentAdmin hiện đúng ghi chú thay vì nút hành động. **Bug thật phát hiện trong lúc viết script test** (không phải bug sản phẩm): selector `a:has-text("Admin")` khớp nhầm sang link tên user "adminuser1" (text match của Playwright là substring không phân biệt hoa/thường) — sửa bằng `a[href="/admin"]`, rút kinh nghiệm cho các test sau. Light/dark/mobile đều đúng (sidebar rơi thành thanh ngang trên mobile). Đã dọn sạch dữ liệu test (3 user + Language/Course/CourseEnrollment), xác nhận không đụng 4 tài khoản có sẵn từ trước.
  - Commit: client `[V21]`, docs `[V40]` (dự kiến — chờ xác nhận từ người dùng trước khi commit/push).

**Bug fix ngoài kế hoạch — Navbar bị vỡ layout (2026-08-08).** Người dùng báo cáo: các mục "Language Learning/Khoá học/Deck/Dashboard..." ở header bị dồn hết về góc trên-trái. **Nguyên nhân**: `components/common/Navbar.tsx` từ Giai đoạn 2 vẫn dùng class Bootstrap thô (`navbar navbar-expand navbar-light`, `nav-link`, `navbar-brand`, `btn btn-outline-secondary`...) — nhưng theo quyết định kiến trúc mục 8.1, `src/styles/_bootstrap.scss` **cố tình không import CSS component** của Bootstrap (`.navbar`/`.nav-link`/`.btn`...), chỉ có grid+reboot+utility chọn lọc. Navbar là component duy nhất còn sót lại chưa redesign nên chưa lộ ra vấn đề này cho tới khi có đủ nhiều mục ở header để thấy rõ layout vỡ.
- Redesign hoàn toàn `Navbar.tsx`+`Navbar.module.scss` mới theo đúng Design System: sticky header, `Logo`+`ThemeToggle` tái dùng có sẵn, nav link dùng `NavLink` (style active), khu vực phải gồm badge Admin (nếu `isAdmin`)/avatar+tên (fallback chữ cái đầu, cùng pattern `ProfilePage`)/nút Đăng xuất (`Button` component), responsive: nav link tự xuống hàng dưới + cuộn ngang trên mobile thay vì hamburger menu (giữ đơn giản, nhất quán tinh thần "không over-engineer" đã áp dụng xuyên suốt).
- Verify trực tiếp bằng Playwright screenshot (không phải chunk mới nên không seed dữ liệu lớn): trạng thái ẩn danh/đã đăng nhập/admin, light/dark/mobile đều đúng, không cần cập nhật `docs/testing/` vì không đổi hành vi nghiệp vụ, chỉ đổi lớp trình bày của 1 component dùng chung.
- Commit: client `[V22]`, docs `[V41]` (dự kiến — chờ xác nhận từ người dùng trước khi commit/push).

- **Chunk 12 — Landing Page (CHUNK CUỐI của Redesign Frontend)** (`pages/LandingPage.tsx`, route `/`). Trang chủ công khai, không yêu cầu đăng nhập.
  - 4 section: **Hero** (gradient primary→secondary + 2 blob mờ tái dùng đúng pattern đã có ở `AuthLayout`, headline + subheadline + 2 CTA); **Feature grid** (6 thẻ giới thiệu tính năng cốt lõi — Vocabulary Learning/Deck+Flashcard/Review SRS/Quiz/Progress tracking/Đa ngôn ngữ — chỉ mô tả, không link); **Khoá học nổi bật** (dữ liệu **thật** qua `GET /api/courses?page=0&size=4`, tái dùng `CourseCard`/`CourseCardSkeleton` có sẵn, **tự ẩn cả section** nếu hệ thống chưa có Course nào thay vì hiện trạng thái rỗng xấu trên trang chủ); **CTA cuối trang** (chỉ hiện khi chưa đăng nhập).
  - **CTA thích ứng theo trạng thái đăng nhập**: đã đăng nhập → nút chính đổi thành "Vào Dashboard" (`/dashboard`) và ẩn hẳn CTA cuối trang; chưa đăng nhập → "Đăng ký miễn phí" (`/register`). Cố tình **không** tạo testimonial/số liệu giả (vd "10,000+ học viên") — chỉ dùng nội dung mô tả tính năng thật và dữ liệu Course thật, tránh nội dung marketing sai sự thật trên sản phẩm thật.
  - Override màu nút CTA trên nền gradient bằng `!important` trong module SCSS riêng của trang (cùng pattern đã dùng ở `ReviewPage` cho `Badge` trên nền gradient Flashcard) — nút chính nền trắng chữ primary tương phản mạnh, nút phụ dạng outline kính mờ.
  - Test E2E qua Playwright + Backend thật (test cả 2 trạng thái: DB không có Course nào — xác nhận section "Khoá học nổi bật" ẩn đúng; sau đó seed 3 Course PUBLISHED — xác nhận section hiện đúng dữ liệu thật): CTA đổi đúng theo trạng thái đăng nhập, ẩn/hiện đúng section theo dữ liệu, Light/dark/mobile đều đúng (feature grid rút cột, hero heading co chữ, CTA button xếp dọc). Đã dọn sạch dữ liệu test (1 admin + 3 Course + 1 Language), xác nhận không đụng 4 tài khoản có sẵn từ trước.
  - Commit: client `[V23]`, docs `[V42]` (dự kiến — chờ xác nhận từ người dùng trước khi commit/push).

**Redesign toàn bộ Frontend theo Design System: ✅ HOÀN THÀNH (2026-08-08).** Đủ 12/12 màn hình theo đúng thứ tự người dùng đã chốt ở đầu mục này, cộng thêm 1 bug fix ngoài kế hoạch (Navbar). Tổng cộng 12 chunk, từ Design System nền (`[V11]`) tới Landing Page (`[V23]`), toàn bộ đều đã test E2E qua Playwright + Backend thật (light/dark/mobile), dọn dữ liệu test sạch sẽ sau mỗi chunk, không còn trang nào ở giao diện Bootstrap mặc định cũ.

**Admin — CRUD nội dung qua giao diện (bắt đầu 2026-08-08, phát sinh ngoài kế hoạch redesign gốc).** Người dùng nhận thấy trang `/admin` (chunk 11) chỉ có Dashboard + Quản lý User hoạt động, các thẻ thống kê khác (Khoá học/Bài học/Từ vựng/Deck/Quiz) không bấm được và không có trang quản lý tương ứng — đúng như giới hạn phạm vi đã ghi nhận ở Chunk 11 (cố tình hoãn CRUD nội dung). Xác nhận với người dùng: làm tiếp CRUD nội dung qua giao diện, chia 3 chunk nhỏ theo đúng nhóm domain Backend: **Chunk A — Language + Course + Lesson** (làm trước, vì Course/Lesson là nền tảng để test 2 chunk sau) → Chunk B — Vocabulary + Grammar → Chunk C — Question (ngân hàng câu hỏi Quiz).

- **Chunk A — Admin Language + Course + Lesson: ✅ Hoàn thành (2026-08-08).** 4 trang mới (`pages/admin/{AdminLanguageListPage,AdminCourseListPage,AdminCourseDetailPage,AdminLessonDetailPage}.tsx`): `AdminLanguageListPage` (`/admin/languages`) — danh sách + tạo mới + sửa (tên/ảnh cờ/trạng thái, `code` không sửa được sau khi tạo đúng theo Backend) + xoá; `AdminCourseListPage` (`/admin/courses`) — danh sách phân trang (Admin thấy **mọi trạng thái** kể cả DRAFT, khác trang Course công khai) + tạo mới (tự điều hướng sang trang chi tiết sau khi tạo vì Course mới luôn ở DRAFT, cần cấu hình tiếp); `AdminCourseDetailPage` (`/admin/courses/:id`) — sửa toàn bộ field + đổi trạng thái (DRAFT/PUBLISHED/ARCHIVED) + xoá Khoá học, kèm danh sách Bài học (Admin thấy mọi trạng thái, khác `LessonSummaryResponse` công khai chỉ PUBLISHED) và form thêm Bài học mới; `AdminLessonDetailPage` (`/admin/lessons/:id`) — sửa field + đổi trạng thái + xoá Bài học, có ghi chú "Quản lý Từ vựng/Ngữ pháp sẽ có ở bước tiếp theo" (để dành Chunk B). Cập nhật `AdminLayout` thêm 2 mục nav "Khoá học"/"Ngôn ngữ". Tái dùng nguyên `CourseResponse`/`CourseSummaryResponse`/`LessonResponse`/`LessonSummaryResponse`/`LanguageResponse` đã có ở `types/course.ts`/`lesson.ts`/`language.ts` (Backend dùng chung DTO cho public lẫn admin), chỉ bổ sung các `*CreateRequest`/`*UpdateRequest` mới; gộp các method admin vào thẳng `languageService.ts`/`courseService.ts`/`lessonService.ts` sẵn có thay vì tạo service riêng (cùng cách `deckService.ts` đã làm ở chunk Deck).
  - Test E2E qua Playwright + Backend thật, đi hết vòng đời thật: tạo Ngôn ngữ mới → sửa tên → tạo Khoá học (chọn ngôn ngữ vừa tạo) → publish Khoá học → thêm Bài học vào Khoá học → publish Bài học → **quay lại kiểm tra trên trang Course công khai thật (`/courses/:id`) xác nhận Bài học vừa tạo qua Admin hiển thị đúng cho user thường** (bước quan trọng nhất, xác nhận toàn vòng khép kín Admin→Public). **Bug thật phát hiện trong lúc viết script test** (không phải bug sản phẩm): cả form sửa Khoá học và form thêm Bài học cùng hiển thị đồng thời trên `AdminCourseDetailPage`, cả 2 đều có input `name="title"` (do `react-hook-form` cùng tên field) — script test dùng selector không đủ cụ thể đã điền nhầm giá trị vào ô "Tên khoá học" thay vì "Tên bài học"; **đã xác minh trực tiếp trong DB rằng dữ liệu Course không hề bị ảnh hưởng** (form chỉ đổi state chưa lưu, không bấm "Lưu thay đổi" sau đó) — sửa script bằng cách chỉ định rõ `nth(1)`, không phải sửa code sản phẩm. Light/dark/mobile đều đúng. Đã dọn sạch dữ liệu test (1 admin + 1 Language + 1 Course + 1 Lesson), xác nhận không đụng các tài khoản có sẵn của người dùng (bao gồm `admin2` người dùng tự tạo trong lúc tìm hiểu cách cấp quyền Admin).
  - Commit: client `[V24]`, docs `[V44]` (dự kiến — chờ xác nhận từ người dùng trước khi commit/push).

Bước tiếp theo: **Chunk B — Admin Vocabulary + Grammar** (CRUD Từ vựng qua giao diện + gắn/gỡ Từ vựng vào Bài học + tạo/sửa Ngữ pháp kèm example, mở rộng `AdminLessonDetailPage` để quản lý 2 phần này ngay tại đó).
