...
> **Trạng thái implement (2026-07-31, cập nhật sau khi Progress Dashboard/XP ra mắt ở Giai đoạn 7):** TC-COURSE-001 → 011, 013 → 018, 021, 022 (Course/Lesson Admin CRUD + xem public, filter/pagination, DRAFT không lộ, sửa Vocabulary hệ thống phản ánh ngay vào Lesson qua join `LessonVocabulary`, Enroll idempotent, Complete Lesson idempotent + tính đúng `progressPercent`/chuyển `COMPLETED`, preview khi chưa enroll) **đã test được**. TC-COURSE-014's XP claim (`LESSON_COMPLETED` trong XpLog) chỉ **đã test được** kể từ retrofit ở Giai đoạn 7 (`docs/dev/SCHEMA_CHANGE_LOG.md`) — trước đó (khi note này viết lần đầu 2026-07-29) XP chưa cộng, chunk Lesson complete lúc đó chỉ tính đúng `progressPercent`. TC-COURSE-017 (Continue Learning) nay **đã test được** — qua `GET /api/progress/dashboard` field `continueLearning` (không phải qua Course Detail như câu chữ gốc "Vào Dashboard, bấm Continue Learning" đã mô tả đúng). TC-COURSE-012 **test được một phần** — `GET /api/lessons/{id}` đã gating đúng theo Enroll và trả đầy đủ Vocabulary + Grammar (kèm example) đúng thứ tự `displayOrder` khi đã enroll, nhưng phần "Question" chưa có (Quiz thuộc Giai đoạn 4). TC-COURSE-020 (Course Progress hiển thị % ở Course Detail) **chưa test được** — `GET /api/courses/{id}` (`CourseResponse`) chưa trả `progressPercent` riêng theo currentUser, chỉ có trên `CourseEnrollmentResponse` (trả về từ enroll/dashboard); cố tình chưa làm, cân nhắc khi Course Detail cần hiển thị tiến độ. TC-COURSE-019 (audio phát âm) thuộc phạm vi khác, chưa test.
