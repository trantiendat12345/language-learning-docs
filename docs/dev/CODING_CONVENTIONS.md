...
- **Fetch dữ liệu trên trang public nhưng gating theo user (chốt 2026-08-07, phát hiện qua bug thật ở Lesson Detail):** trang nằm ngoài `ProtectedRoute` mà nội dung/gating phụ thuộc `currentUserId` (vd field `enrolled` trên `LessonResponse`/`CourseResponse`) **phải** đợi `useAuthContext().isLoading === false` rồi mới gọi API, và đưa giá trị này (đổi tên khi destructure, vd `isLoading: isAuthLoading`, để không đụng state loading riêng của page) vào dependency array của effect fetch. Không cần áp dụng cho trang nằm trong `ProtectedRoute` (đã tự chặn render tới khi `AuthContext` khôi phục xong). Lý do: fetch ngay khi mount mà không đợi sẽ khiến request đầu tiên sau khi tải lại trang (F5) chạy trước khi `accessToken` kịp phục hồi từ `refreshToken` cookie → Backend nhận request ẩn danh, trả sai dữ liệu/gating cho user đã đăng nhập, và do effect không tự chạy lại nên trang bị "kẹt" ở trạng thái sai vĩnh viễn tới khi F5 lại lần nữa.
